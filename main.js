document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- СОЗДАЕМ ИНТЕРФЕЙС (HTML ПОВЕРХ ИГРЫ) ---
    // Плашки со статистикой
    const statsContainer = document.createElement('div');
    statsContainer.className = 'game-stats-ui';

    const scoreUI = document.createElement('div');
    scoreUI.className = 'stat-badge';
    const goldUI = document.createElement('div');
    goldUI.className = 'stat-badge';
    const livesUI = document.createElement('div');
    livesUI.className = 'stat-badge';

    statsContainer.appendChild(scoreUI);
    statsContainer.appendChild(goldUI);
    statsContainer.appendChild(livesUI);
    ...
    // Переменные для отслеживания изменений (чтобы не перерисовывать UI просто так)
    ...
    function updateUI() {
        ...
        if (lastScore !== score || lastCoins !== coins || lastLives !== lives || lastArchers !== archers) {
            ...
            if (coins >= archerCost && !isGameOver) {
                ...
                shopBtn.style.boxShadow = '0 0 15px rgba(255, 215, 0, 0.8)'; // Свечение, когда хватает денег
            } else {
    ...
    shopBtn.addEventListener('click', (e) => {
        ...
        if (coins >= archerCost) {
            ...
            updateUI(); // Принудительно обновляем UI сразу после покупки
    ...
    function initGame() {
        ...
        // Сброс переменных UI, чтобы они точно обновились на старте
        lastScore = -1; lastCoins = -1; lastLives = -1; lastArchers = -1;
        
        shopBtn.style.display = 'block'; 
        statsContainer.style.display = 'flex'; // Показываем интерфейс
        updateUI();
    ...
    function update() {
        ...
        // Вызываем обновление UI в конце каждого кадра (обновится только если изменились значения)
        updateUI();
    }
    ...
    function draw() {
        ...
        // ВНИМАНИЕ: Отрисовку текста из Canvas мы полностью удалили!
    }
    ...
    function endGame() {
        ...
        shopBtn.style.display = 'none'; 
        statsContainer.style.display = 'none'; // Прячем интерфейс при проигрыше
